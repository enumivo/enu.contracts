...
#include <enu.system/rex.results.hpp>
...
namespace enumivosystem {
   ...
   void system_contract::buyrex( const name& from, const asset& amount )
   {
      ...
      rex_results::buyresult_action buyrex_act( rex_account, std::vector<enumivo::permission_level>{ } );
   ...
   void system_contract::sellrex( const name& from, const asset& rex )
   {
      ...
      if ( current_order.success ) {
         rex_results::sellresult_action sellrex_act( rex_account, std::vector<enumivo::permission_level>{ } );
   ...
   int64_t system_contract::update_renewed_loan( Index& idx, const Iterator& itr, int64_t rented_tokens )
   {
      ...
      idx.modify ( itr, same_payer, [&]( auto& loan ) {
         ...
         loan.expiration         += enumivo::days(30);
   ...
   void system_contract::runrex( uint16_t max )
   {
      ...
      /// transfer from enu.names to enu.rex
         ...
         for ( uint16_t i = 0; i < max; ++i ) {
            ...
            if ( bitr != _rexbalance.end() ) { // should always be true
               ...
               if ( result.success ) {
                  ...
                  rex_results::orderresult_action order_act( rex_account, std::vector<enumivo::permission_level>{ } );
   ...
   int64_t system_contract::rent_rex( T& table, const name& from, const name& receiver, const asset& payment, const asset& fund )
   {
      ...
      table.emplace( from, [&]( auto& c ) {
         ...
         c.expiration   = current_time_point() + enumivo::days(30);
         c.loan_num     = pool->loan_num;
      });

      rex_results::rentresult_action rentresult_act{ rex_account, std::vector<enumivo::permission_level>{ } };
   ...
   void system_contract::channel_to_rex( const name& from, const asset& amount )
   {
      ...
      if ( rex_available() ) {
         ...
         transfer_act.send( from, rex_account, amount,
                            std::string("transfer from ") + from.to_string() + " to enu.rex" );
      }
#endif
   }
   ...
   {
      /**
       * If CORE_SYMBOL is (ENU,4), maximum supply is 10^10 tokens (10 billion tokens), i.e., maximum amount
