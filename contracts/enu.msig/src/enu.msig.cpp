                        ...
                        ignore<transaction> trx )
{
   ...
   check( _trx_header.expiration >= enumivo::time_point_sec(current_time_point()), "transaction expired" );
...
void multisig::cancel( name proposer, name proposal_name, name canceler ) {
   ...
   if( canceler != proposer ) {
      check( unpack<transaction_header>( prop.packed_transaction ).expiration < enumivo::time_point_sec(current_time_point()), "cannot cancel until expiration" );
   }
...
void multisig::exec( name proposer, name proposal_name, name executer ) {
   ...
   ds >> trx_header;
   check( trx_header.expiration >= enumivo::time_point_sec(current_time_point()), "transaction expired" );
