...
>>>>>>> upstream/master:tests/eosio.system_tester.hpp
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(enu.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue(config::system_account_name, asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "enumivo", core_symbol ) );
   }
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("enumivo")  + get_balance("enu.ramfee") + get_balance("enu.stake") + get_balance("enu.ram") );
   }
   ...
   enu_system_tester( setup_level l = setup_level::full ) {
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(enu.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   fc::variant get_global_state2() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "enumivo_global_state2", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state3() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "enumivo_global_state3", data, abi_serializer_max_time );
   }
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( "enumivo", "enu.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "enumivo", "alice1111111", core_sym::from_string("650000000.0000"), "enumivo" );
...
inline uint64_t M( const string& enu_str ) {
   return core_sym::from_string( enu_str ).get_amount();
}
